...
// Photo-derived (two dither styles) beside the hand-drawn version.
{
  const { ART } = await import('../games/explorer/art.js');
  const photo = load('data/photos/hoover-tower.json');
  const credit = `${photo.credit.artist} / ${photo.credit.license}`.slice(0, 25);
  const body = ['A 285-foot bell tower', 'finished in 1941.'];

  const variants = [
    panel({ w: photo.w, h: photo.h, bits: photo.bits }, 'PHOTO / ORDERED', credit, body),
    panel({ w: photo.w, h: photo.h, bits: photo.bitsFloyd }, 'PHOTO / DIFFUSED', credit, body),
    panel(ART.hooverTower, 'HAND DRAWN', '', body),
  ];
  for (const lookId of ['dmg', 'color']) {
    const m = montage(variants.map((v) => toRGB(v, LOOKS[lookId])), 3, 6);
    writeFileSync(resolve(OUT, `art-hoover-${lookId}.png`), encodePNG(m.w, m.h, m.rgb, 3));
  }
  const sent = panel(ART.sentinel, 'THE SENTINEL', '', ['Albert Paley, 2003.', 'Seventy feet of steel.']);
